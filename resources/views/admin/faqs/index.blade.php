...
<div class="container-xxl flex-grow-1 container-p-y">
    ...
    <x-admin.table :headers="['#', __('dashboard.question'), __('dashboard.status'), __('dashboard.actions')]" :createRoute="$createRoute" :title="$title" :buttonText="__('dashboard.add')" :search="true" :indexRoute="$route">
        @forelse($models as $model)
            <tr>
                <td>{{ $loop->iteration }}</td>
                <td>{{ $model->getDisplayTranslation('question') }}</td>
